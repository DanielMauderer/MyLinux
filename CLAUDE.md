...
2. Matugen writes to: `hypr/colors.lua` (Hyprland palette module), `waybar/matugen.css`, `wlogout/matugen.css`, `kitty/colors.conf`, `dunst/dunstrc`, `rofi/colors.rasi`
3. Each app's style imports the generated file alongside a static `mocha.css` or base config

**Key color variables** — `hypr/colors.lua` returns a table of `rgba(RRGGBBAA)` strings, consumed with `local c = require("colors")`:
- `c.primary`, `c.secondary`, `c.tertiary`, `c.error`
- `c.surface`, `c.on_surface`, `c.on_surface_variant`, `c.outline`
- `c.surface_container`, `c.surface_container_high`, etc.
...
Hyprland 0.56 removed hyprlang: the config is **Lua** (`hl.*` API). `hypr/hyprland.lua` is
the entry point and pulls in modules with `require()`. The Lua search path is rooted at
`~/.config/hypr`, so `require("conf.monitor")` loads `hypr/conf/monitor.lua`. The old `.conf`
tree is dead weight — Hyprland prefers `hyprland.lua` when both exist.

| File | Purpose |
|------|---------|
| `conf/monitor.lua` | Monitor layout (requires a profile from `conf/monitors/`) |
| `conf/workspace.lua` | Workspace rules (requires a profile from `conf/workspaces/`) |
| `conf/keybinding.lua` | Requires `conf/keybindings/default.lua` |
| `conf/decoration.lua` / `conf/animation.lua` | Visual effects |
| `conf/windowrule.lua` | App-specific window rules |
| `conf/ml4w.lua` | ML4W window rules + environment variables |
| `conf/focus-mode-rules.lua` | Rules for focus/zen mode (rewritten by `focus-mode.sh`) |

Validate any change without touching the running session:

```bash
Hyprland --verify-config -c ~/.config/hypr/hyprland.lua
```

It catches unknown config keys, bad dispatcher arguments and unknown keysyms, so it is the
fastest way to check a port. `hyprctl dispatch` and `hyprctl eval` now take Lua
(`hyprctl dispatch 'hl.dsp.exit()'`), and binds carry a `description` that
`scripts/keybindings.sh` reads back out of `hyprctl binds -j`.
...
- **New Hyprland keybinding**: Edit `hypr/conf/keybindings/default.lua` (`bind`/`exec` helpers take a description); verify with `Hyprland --verify-config -c ~/.config/hypr/hyprland.lua`, reload with `SUPER+CTRL+R`
